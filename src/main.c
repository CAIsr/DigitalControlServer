...
int		exec_command(t_cam *c, char *command, char **param)
{
  ...
  if (c->cam_usb && c->cam_usb->pluggued != 0)
    {
    ...
    creat_and_send_message(KO, command, NULL, "Camera ejected or disconnected", c);
  return (0);
}
...
void		*input_remote_control(void *a)
{
  int fd;
...
int		main(int ac, char **av)
{
  ...
  /*  pthread_create(&remote_thread, NULL, input_remote_control, (void *)c);
  ...
  if (ac == 2)
    {
      c->sock_type = SOCK_TCP;
      init_comm_tcp(c, atoi(av[1]));
    }
  else
    {
      c->sock_type = SOCK_UNIX;
