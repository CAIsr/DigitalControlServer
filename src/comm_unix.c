...
char		*creat_message(int code, char *command, char **param, char *message)
{
  ...
  if (param != NULL)
    ...
    asprintf(&final_message, "%s|NULL|", final_message);
				       ...
				       char **param, char *message, t_cam *c)
{
  ...
  if (c->sock_type == 1) /* SOCK_UNIX */
    {
      write(c->active_sock, msg, strlen(msg));
    }
  if (c->sock_type == 2) /* SOCK_TCP */
    {
      if (code == WAIT_RESPONSE)
	{
	  if (c->msg_tmp_tcp != NULL)
	    {
	      asprintf(&tmp, "%s\n%s", c->msg_tmp_tcp, msg);
	      free(msg);
	      free(c->msg_tmp_tcp);
      ...
      else
	{
	  ...
	  write_http_header(c->active_sock, "200", msg);
	  close(c->active_sock);
	  free(c->msg_tmp_tcp);
	  c->msg_tmp_tcp = NULL;
	}
...
void		serv_accept_new_connections(t_serv_comm *s)
{
  ...
  if ((socket = accept(s->sock_serv, (struct sockaddr*)&client_addr,
		       &len)) == -1)
    {
      fprintf(stderr, "UNIX Accept Error");
      s->state = FAIL;
      return;
    }
  ...
  if (s->c->camera_value_list)
    {
      asprintf(&header, "||%*u||", 8, (unsigned int)strlen(s->c->camera_value_list));
...
int		serv_init_connect_unix(t_serv_comm *s, char *path)
{
  if ((s->sock_serv = socket(AF_UNIX, SOCK_STREAM, 0)) < 0)
    {
      fprintf(stderr, "UNIX Socket creation Error");
      return (-1);
    }
  s->queue_size = 50;
  unlink(path);
  ...
  if (bind(s->sock_serv, (struct sockaddr*)&(s->serv_addr),
    ...
    {
      fprintf(stderr, "Unix Bind Error");
      return (-1);
    }
